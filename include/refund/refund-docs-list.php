<li>
    <a href="Заявление_на_возврат_ДС.docx" class="doc-link" download target="_blank">
        <img src="/local/templates/main/assets/img/doc-file-icon.svg" width="25" alt="">
        <span class="doc-link-body">
            <span class="doc-link-title">Заявление на возврат денежных средств</span>
            <span class="doc-link-size">Скачать: 14 Кб</span>
        </span>
    </a>
</li>
<li>
    <a href="Заявление_на_возврат_товара_покупателя.doc" class="doc-link" download target="_blank">
        <img src="/local/templates/main/assets/img/doc-file-icon.svg" width="25" alt="">
        <span class="doc-link-body">
            <span class="doc-link-title">Заявление на возврат товара покупателя</span>
            <span class="doc-link-size">Скачать: 27 Кб</span>
        </span>
    </a>
</li>