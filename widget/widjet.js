...
function ISDEKWidjet(params) {
    ...
    var defaultCity = sessionStorage.getItem('city');
	...
	var widjet = new ipolWidjetController({
		...
		options: {
			...
			apikey: {
				value: 'c617d6aa-9ec6-4567-8bb9-7b600859bc60',
				check: function (wat) {
					return (typeof(wat) === 'string');
				},
				setting: 'start',
		...
		stages: {
			...
			'ready': {
				...
				function: function () {
                    ...
                    // var defaultCity = sessionStorage.getItem('city');
                    // if(cookiesSplit['prymery.geoip.2.8.1_city_id']){
                    //     $.getJSON('/local/ajax/get_location.php',
                    //         {
                    //             CITY: cookiesSplit['prymery.geoip.2.8.1_city_id'],
                    //             ACTION: 'selectById',
                    //         },
                    //         function (data) {
                    //             DATA.city.set(data);
	...
	var DATA = {
		...
		PVZ: {
			...
			getCurrent: function () {
                ...
                //     DATA.city.current = $('.js-prymery__geoip__city__line-city:first').text();
                // }
	...
	var template = {
		...
		ymaps: {
            ...
            loadMap: function (city) {
                ...
                // if($('.js-prymery__geoip__city__line-city:first').text().length > 0){
                //     city = $('.js-prymery__geoip__city__line-city:first').text();
                // }
				let mtypes = {};
			...
			placeMarks: function (mtypes) {
				...
				if (typeof pvzList === 'object') {
					...
					for (var i in pvzList) {
						...
						let img = widjet.options.get('path') + ((pvzList[i].Postamat != true) ? 'images/sdekNActive.png' : null);
						let imgActive = widjet.options.get('path') + ((pvzList[i].Postamat != true) ? 'images/sdekActive.png' : null);
