...
$explode_url = explode('/',$item['DETAIL_PAGE_URL']);
$item['DETAIL_PAGE_URL'] = '/product/'.$explode_url[count($explode_url)-2].'/';

?>
<div class="product-item">
    <div class="product-item__labels">
        <? $element->echoStickers(); ?>
        <?if(array_first($item['OFFERS'][0]['PRICES'])['DISCOUNT_DIFF']>0):?>
	...
	<? endif; ?>
        ...
        <?
        if($item['PREVIEW_PICTURE_SECOND']['ID']){
            $file = CFile::ResizeImageGet($item['PREVIEW_PICTURE_SECOND']['ID'], array('width'=>250, 'height'=>250), BX_RESIZE_IMAGE_PROPORTIONAL, true);
            $bgImage = $file['src'];
        }elseif($item['PREVIEW_PICTURE']['ID']){
