	...
	BX.Sale.OrderAjaxComponent = {
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "zipHide bx-soa-location-input-container"}});
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, labelNodes2;
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y')
			{
                ...
                if(split_name[1]){
                    var split_name2 = split_name[1].split(')');
                    item.NAME = split_name[0]+'<span>('+split_name2[0]+')</span>'+split_name2[1];
                }
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company order__delivery-service deliveryId_'+item.ID},
				children: [label, title, labelNodes2],
		...
		showValidationResult: function(inputs, errors)
		{
			...
			if (errors.length)
				...
				this.showErrorTooltip(tooltipId, '', errors.join('<br>'));
			else
		...
		createTotalUnit: function(name, value, params)
		{
			...
			else if (params.free)
			{
				totalValue = [BX.create('SPAN', {
					props: {className: 'bx-soa-price-free sale-color'},
					html: value
				})];
			}
			else
