                ...
                if (!!r.response) {
                    ...
                    $('.auth-inputs__input:last-child').on('keyup', function(e) {
                        if($(this).val() != '') {
                            ...
                            $('.smscode').val($('.auth-inputs__input:first-child').val()+''+$('.auth-inputs__input:nth-child(2)').val()+''+$('.auth-inputs__input:nth-child(3)').val()+''+$('.auth-inputs__input:last-child').val());
                            $('input[name=password_sms_code]').val($('.auth-inputs__input:first-child').val()+''+$('.auth-inputs__input:nth-child(2)').val()+''+$('.auth-inputs__input:nth-child(3)').val()+''+$('.auth-inputs__input:last-child').val());
                            $('.btnAfterSms').click();
                        }
                    });
                    ...
                    if (!!r.response.timeout) {
                        ...
                        var smsInterval = setInterval(function () {
                            if (--timeout > 0) {
                                ...
                                $('.cbaup_btn_link.timeout').text('Отправить код ещё раз через ' + timeout+' сек.');
                            } else {
                                clearInterval(smsInterval);
                                $('.cbaup_btn_link.timeout').text(self.getMessage('BTN_SEND_CODE'));
                    ...
                    if(r.error.code != 'ERROR_INVALID_PHONE'){
                        self.showMsg(r.error.msg, true);
                    }
                    //captcha
                    if (!!r.error.more.captchaId) {
